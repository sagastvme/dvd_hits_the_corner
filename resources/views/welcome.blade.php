            ...
            function moveTop(i, direction_top) {
                setTimeout(() => {
                    if (i == window.innerHeight-70) {
                        ...
                        img.style.top = i + 'px';
                        if(direction_top==='down'){
                            moveTop(i + 1,direction_top); // Move to the next position
                        }else{
                            moveTop(i-1,direction_top);
                        }
                }, 5);
            }
            function moveImage(i) {
                setTimeout(() => {
                    ...
                    if(direction_horizontal==='right'){
                        moveImage(i + 1,direction_horizontal); // Move to the next position
                    }else{
                        moveImage(i-1,direction_horizontal);
                    }

                }, 5);
            }

            moveImage(1, direction_horizontal); // Start the loop with i = 0
moveTop(1, direction_top)
...
<body>
<svg id="dvd" xmlns="http://www.w3.org/2000/svg"
     width="125px"  viewBox="0 0 210 107">
    <!-- Your SVG code here -->
